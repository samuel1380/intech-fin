...
export const getActiveProviderName = () => 'Consultor financeiro';
export const getActiveModelName = () => 'Conexão protegida';
...
const callAI = async (
  ...
  if (error || typeof data?.text !== 'string')
    throw new Error(
      'Não foi possível consultar a IA. Verifique a conexão e a configuração do serviço.',
    );
  return data.text;
};
...
export const buildFinancialContext = (
  ...
  const categoryStr = Object.entries(categoryBreakdown)
    ...
    .map(
      ([cat, data]) =>
        `  - ${cat}: ${data.count} transações | Receita: R$${data.income.toFixed(2)} | Despesa: R$${data.expense.toFixed(2)}`,
    )
    .join('\n');
  ...
  const employeeData: Record<
    string,
    { totalRevenue: number; totalCommission: number; jobs: number }
  > = {};
  transactions
    ...
    .forEach((t) => {
      ...
      if (!employeeData[name])
        employeeData[name] = { totalRevenue: 0, totalCommission: 0, jobs: 0 };
      employeeData[name].jobs++;
      if (t.type === TransactionType.INCOME)
        employeeData[name].totalRevenue += t.amount;
  ...
  const employeeStr =
    Object.entries(employeeData)
      .map(
        ([name, data]) =>
          `  - ${name}: ${data.jobs} serviços | Gerou R$${data.totalRevenue.toFixed(2)} | Comissão: R$${data.totalCommission.toFixed(2)}`,
      )
  ...
  const pendingPayments =
    transactions
      ...
      .map(
        (t) =>
          `  - ${t.description}: R$${t.pendingAmount!.toFixed(2)} pendente (de R$${t.amount.toFixed(2)})`,
      )
  ...
  const upcomingCommissions =
    transactions
      ...
      .sort((a, b) =>
        a.commissionPaymentDate!.localeCompare(b.commissionPaymentDate!),
      )
      .slice(0, 5)
      .map(
        (t) =>
          `  - ${t.employeeName || 'Técnico'}: R$${t.commissionAmount!.toFixed(2)} em ${t.commissionPaymentDate}`,
      )
  ...
  return callAI([
    {
      role: 'system',
      content:
        'Você é um CFO virtual especializado em empresas de desentupidora no Brasil. Responda em português formal com dados reais. Texto puro, sem markdown.',
    },
