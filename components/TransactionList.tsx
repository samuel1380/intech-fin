...
const TransactionList: React.FC<Props> = ({
  ...
  const [showModal, setShowModal] = useState(!!initialType);
  ...
  const handleEdit = (t: Transaction) => {
    ...
    setEditingId(t.id);
    setFormData({
      serviceDate: t.serviceDate || t.date,
      isInstallment: false,
      installCount: '2',
      installInterval: '30',
      ...
      recurringDay:
        t.recurringDay?.toString() || new Date().getDate().toString(),
    });
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const baseData: any = {
        ...
        pendingAmount:
          formData.pendingAmount && parseFloat(formData.pendingAmount) > 0
            ? parseFloat(formData.pendingAmount)
            : undefined,
        isRecurring: formData.isRecurring || undefined,
        recurringIntervalMonths: formData.isRecurring
          ? parseInt(formData.recurringIntervalMonths)
          : undefined,
        recurringDay: formData.isRecurring
          ? parseInt(formData.recurringDay)
          : undefined,
      };
      ...
      if (isEditing && editingId) {
        ...
        if (formData.isInstallment && !formData.isRecurring) {
          const count = Math.max(
            1,
            Math.min(120, parseInt(formData.installCount) || 1),
          );
          const interval = Math.max(
            1,
            Math.min(365, parseInt(formData.installInterval) || 30),
          );
          const totalAmount = baseData.amount;
          const totalCommission = baseData.commissionAmount || 0;
          const totalPending =
            baseData.pendingAmount !== undefined
              ? baseData.pendingAmount
              : baseData.status === TransactionStatus.PENDING
                ? totalAmount
                : 0;

          const amountPerInstallment =
            Math.floor(Math.round(totalAmount * 100) / count) / 100;
          const commissionPerInstallment =
            Math.floor(Math.round(totalCommission * 100) / count) / 100;
          const pendingPerInstallment =
            Math.floor(Math.round(totalPending * 100) / count) / 100;
          ...
          const firstDueDate = new Date(y, m - 1, d);

          const installments: Omit<Transaction, 'id'>[] = [];
          for (let i = 1; i <= count; i++) {
            const isLast = i === count;
            const instAmount = isLast
              ? parseFloat((totalAmount - createdAmount).toFixed(2))
              : amountPerInstallment;
            const instCommission =
              isLast && totalCommission > 0
                ? parseFloat((totalCommission - createdCommission).toFixed(2))
                : commissionPerInstallment;
            const instPending =
              isLast && totalPending > 0
                ? parseFloat((totalPending - createdPending).toFixed(2))
                : pendingPerInstallment;
            ...
            installments.push({
              ...
              amount: instAmount,
              commissionAmount:
                totalCommission > 0 ? instCommission : undefined,
              pendingAmount: totalPending > 0 ? instPending : undefined,
              date: format(dueDate, 'yyyy-MM-dd'),
            });
          }
          await onAddTransactions(installments);
        } else {
      ...
      notify(`Erro ao salvar: ${error.message || 'Erro desconhecido'}`);
    } finally {
  ...
  const handleMarkAsPaid = async (t: Transaction) => {
    ...
    try {
      ...
      notify(`Erro ao marcar como pago: ${error.message}`);
    } finally {
  ...
  const handleExportCSV = () => {
    ...
    const headers = [
      'ID;Data;Descrição;Categoria;Tipo;Valor;Status;Funcionário;Comissão;Data Pagto Comissão;Valor Pendente;Recorrente',
    ];
    ...
    const rows = filtered.map((t) => {
      ...
      const formattedCommission = t.commissionAmount
        ? t.commissionAmount.toLocaleString('pt-BR', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          })
        : '0,00';
      const formattedPending = t.pendingAmount
        ? t.pendingAmount.toLocaleString('pt-BR', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          })
        : '0,00';
      const recurring = t.isRecurring
        ? `Sim (${t.recurringIntervalMonths}m dia ${t.recurringDay})`
        : 'Não';
    ...
    link.setAttribute(
      'download',
      `transacoes_${format(new Date(), 'dd-MM-yyyy')}.csv`,
    );
  ...
  return (
    <div className="space-y-5 animate-fade-in w-full pb-8">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-5 pb-5">
        <div>
          <h2 className="text-[22px] font-bold text-[#0F172A] dark:text-white tracking-tight">
            Atividades Recentes
          </h2>
          <p className="text-[#94A3B8] dark:text-slate-400 mt-1 font-medium text-[13px]">
            Controle de caixa e histórico financeiro.
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-[32px] shadow-premium dark:shadow-none border border-[#EEF2F7] dark:border-white/[0.06] overflow-hidden w-full flex flex-col h-[650px] p-2">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 px-4 sm:px-6 py-5 border-b border-[#EEF2F7] dark:border-white/[0.06]">
          <h3 className="font-bold text-slate-800 dark:text-white text-[16px]">
            Atividades
          </h3>
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full sm:w-auto">
            ...
            <select
              ...
              className="rounded-full border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 text-sm px-4 py-2"
            >
        ...
        <div className="overflow-auto custom-scrollbar flex-1 relative px-2">
          <table className="hidden md:table w-full text-left border-collapse min-w-[900px]">
            <thead className="bg-white dark:bg-slate-800 sticky top-0 z-10">
              <tr>
                <th className="px-6 py-4 text-[12px] font-semibold text-slate-400 border-b border-slate-100 dark:border-slate-700/50 w-[120px]">
                  Data
                </th>
                <th className="px-6 py-4 text-[12px] font-semibold text-slate-400 border-b border-slate-100 dark:border-slate-700/50">
                  Atividade
                </th>
                <th className="px-6 py-4 text-[12px] font-semibold text-slate-400 border-b border-slate-100 dark:border-slate-700/50 w-[150px]">
                  Categoria
                </th>
                <th className="px-6 py-4 text-[12px] font-semibold text-slate-400 border-b border-slate-100 dark:border-slate-700/50 w-[150px]">
                  Preço
                </th>
                <th className="px-6 py-4 text-[12px] font-semibold text-slate-400 border-b border-slate-100 dark:border-slate-700/50 w-[140px]">
                  Status
                </th>
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-slate-700/30">
              {paginatedTransactions.map((t) => {
                ...
                return (
                  <tr
                    key={t.id}
                    className="hover:bg-[#F8F9FC]/60 dark:hover:bg-white/[0.02] transition-colors duration-150 group table-row-hover"
                  >
                    <td className="px-6 py-4 text-[#64748B] font-medium tabular-nums text-[13px]">
                      {day}/{month}/{year}
                    </td>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {t.isRecurring && (
                          <span className="inline-flex items-center gap-1 text-[9px] font-medium px-1.5 py-0.5 bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 rounded-full border border-purple-100 dark:border-purple-800/40">
                            ...
                            {t.recurringIntervalMonths === 1
                              ? 'Mensal'
                              : `${t.recurringIntervalMonths}m`}
                          </span>
                        )}
                      </div>
                      {t.employeeName && (
                        <div className="text-[10px] text-primary-500 font-medium uppercase mt-1 flex flex-col">
                          <span>Técnico: {t.employeeName}</span>
                          {t.commissionPaymentDate && (
                            <span className="text-[#94A3B8] normal-case font-medium">
                              Pagto Comissão:{' '}
                              {format(
                                new Date(t.commissionPaymentDate + 'T12:00:00'),
                                'dd/MM/yyyy',
                              )}
                            </span>
                          )}
                    ...
                    <td
                      className={`px-6 py-5 font-bold font-mono text-[14px] ${t.type === TransactionType.INCOME ? 'text-slate-800 dark:text-white' : 'text-slate-800 dark:text-white'}`}
                    >
                      {formatBRL(t.amount)}
                    </td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <button
                          ...
                          className="p-2 text-[#94A3B8] hover:text-primary-600 dark:hover:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/20 rounded-[10px] transition-all duration-200 lg:opacity-0 group-hover:opacity-100"
          ...
          <div className="md:hidden divide-y divide-slate-100 dark:divide-slate-800">
            {paginatedTransactions.map((t) => {
              ...
              return (
                <div
                  key={t.id}
                  className="p-4 flex flex-col gap-3 hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition-colors"
                >
                  <div className="flex justify-between items-start gap-2">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-slate-100 dark:bg-slate-700/50 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-600/50 truncate">
                          {t.category}
                        </span>
                      </div>
                      <h4 className="font-bold text-slate-800 dark:text-slate-200 text-sm leading-snug break-words">
                        {t.description}
                      </h4>
                      {t.isRecurring && (
                      ...
                      {t.employeeName && (
                        <div className="text-[10px] text-primary-500 font-bold uppercase mt-1.5 flex flex-col">
                    ...
                    <div className="flex flex-col items-end shrink-0 gap-1.5">
                      <span
                        className={`font-bold font-mono text-base ${t.type === TransactionType.INCOME ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}
                      >
                      ...
                      <button
                        ...
                        className={`inline-flex items-center gap-1 px-2 py-1 rounded text-[10px] font-bold transition-all border shadow-sm ${
                          t.status === TransactionStatus.COMPLETED
                      ...
                      {t.pendingAmount && t.pendingAmount > 0 && (
                        <span className="text-[10px] text-rose-500 font-bold mt-0.5">
                          Pend: {formatBRL(t.pendingAmount)}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-end gap-2 pt-3 mt-1 border-t border-slate-100 dark:border-slate-800/50">
                    ...
                    <button
                      onClick={() => handleEdit(t)}
                      className="flex-1 py-2 px-3 text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-800/50 rounded-lg transition-colors flex items-center justify-center gap-1.5 text-xs font-bold"
                    >
        ...
        {filtered.length > 0 ? (
          ...
          <div className="p-16 text-center text-[#94A3B8] flex flex-col items-center justify-center flex-1">
            ...
            <h3 className="text-[#0F172A] dark:text-slate-200 font-semibold mb-1 text-[15px]">
              Nenhum resultado
            </h3>
          ...
          <div className="bg-white dark:bg-[#0F172A] border border-[#EEF2F7] dark:border-white/[0.06] rounded-[24px] shadow-2xl w-full max-w-lg overflow-hidden animate-fade-in-up max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-5 border-b border-[#EEF2F7] dark:border-white/[0.06] flex justify-between items-center bg-white dark:bg-[#0F172A] sticky top-0 z-10">
              ...
              <button
                ...
                className="p-2 bg-[#F8F9FC] dark:bg-slate-800 hover:bg-[#EEF2F7] dark:hover:bg-slate-700 rounded-full text-[#64748B] dark:text-slate-400 transition-colors duration-200"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="p-1 bg-[#F8F9FC] dark:bg-slate-800/60 rounded-[14px] grid grid-cols-2 gap-1">
                <button
                  ...
                  className={`py-2.5 rounded-[10px] text-[13px] font-semibold transition-all duration-200 flex items-center justify-center gap-2 ${
                ...
                >
                  <span
                    className={`w-2 h-2 rounded-full ${formData.type === TransactionType.INCOME ? 'bg-emerald-500' : 'bg-slate-300'}`}
                  ></span>
                  Receita
                </button>
                <button
                ...
                >
                  <span
                    className={`w-2 h-2 rounded-full ${formData.type === TransactionType.EXPENSE ? 'bg-rose-500' : 'bg-slate-300'}`}
                  ></span>
                  Despesa
                </button>
              </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    required
                    type="text"
                    className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-600 rounded-xl focus:bg-white dark:focus:bg-slate-800 focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition-all font-medium text-slate-800 dark:text-slate-200 placeholder:text-slate-400"
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          const commAmount =
                            val && rate
                              ? (
                                  (parseFloat(val) * parseFloat(rate)) /
                                  100
                                ).toFixed(2)
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5 ml-1">
                      Data do Serviço / Lançamento
                    </label>
                    <input
                      required
                      type="date"
                      className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-600 rounded-xl focus:bg-white dark:focus:bg-slate-800 focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none transition-all font-medium text-slate-700 dark:text-slate-200"
                ...
                {!isEditing && !formData.isRecurring && (
                  <div className="p-4 bg-sky-50/50 dark:bg-sky-900/10 rounded-2xl border border-sky-100 dark:border-sky-800/50 space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      >
                        <div
                          className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow-sm transition-transform duration-200 ${formData.isInstallment ? 'translate-x-5' : ''}`}
                        ></div>
                      </button>
                    </div>
                    ...
                    {formData.isInstallment && (
                      <div className="grid grid-cols-2 gap-4 animate-fade-in">
                          ...
                          <div className="relative">
                            <select
                              ...
                              onChange={(e) =>
                                setFormData({
                                  ...formData,
                                  installInterval: e.target.value,
                                })
                              }
                            >
                        ...
                        <p className="col-span-2 text-[10px] text-sky-500 dark:text-sky-400 italic ml-1 leading-relaxed">
                          * O valor total de{' '}
                          {formatBRL(parseFloat(formData.amount || '0'))} será
                          dividido em {formData.installCount} boletos/parcelas.
                          <br />* O primeiro vencimento será na "Data do
                          Vencimento" preenchida acima (
                    ...
                    <div className="relative">
                      ...
                      >
                        <optgroup label="Serviços de Desentupidora">
                          <option value={TransactionCategory.SERVICE_PIA}>
                            Pia
                          </option>
                          <option value={TransactionCategory.SERVICE_RALO}>
                            Ralo
                          </option>
                          <option value={TransactionCategory.SERVICE_ESGOTO}>
                            Esgoto
                          </option>
                          <option value={TransactionCategory.SERVICE_VASO}>
                            Vaso Sanitário
                          </option>
                          <option
                            value={TransactionCategory.SERVICE_CAIXA_GORDURA}
                          >
                            Caixa de Gordura
                          </option>
                          <option value={TransactionCategory.SERVICE_COLUNA}>
                            Coluna de Prédio
                          </option>
                          <option
                            value={TransactionCategory.SERVICE_HIDROJATEAMENTO}
                          >
                            Hidrojateamento
                          </option>
                          <option value={TransactionCategory.SERVICE_OUTROS}>
                            Outros Serviços
                          </option>
                        </optgroup>
                        <optgroup label="Outras Categorias">
                          <option value={TransactionCategory.SALES}>
                            Vendas
                          </option>
                          <option value={TransactionCategory.OPERATIONS}>
                            Operacional
                          </option>
                          <option value={TransactionCategory.PAYROLL}>
                            Folha de Pagamento
                          </option>
                          <option value={TransactionCategory.MARKETING}>
                            Marketing
                          </option>
                          <option value={TransactionCategory.TAXES}>
                            Impostos
                          </option>
                          <option value={TransactionCategory.SOFTWARE}>
                            Software/SaaS
                          </option>
                          <option value={TransactionCategory.OFFICE}>
                            Escritório
                          </option>
                          <option value={TransactionCategory.INVESTMENT}>
                            Investimentos
                          </option>
                          <option value={TransactionCategory.PERSONAL}>
                            Pessoal
                          </option>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      >
                        ...
                        <option value={TransactionStatus.PARTIAL}>
                          Pagto Parcial
                        </option>
                      </select>
                      <ArrowUpDown className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 pointer-events-none" />
                    ...
                    {formData.isRecurring && (
                      <div className="grid grid-cols-2 gap-4 animate-fade-in">
                          ...
                          <div className="relative">
                            <select
                              ...
                              onChange={(e) =>
                                setFormData({
                                  ...formData,
                                  recurringIntervalMonths: e.target.value,
                                })
                              }
                            >
                        ...
                        <p className="col-span-2 text-[10px] text-purple-500 dark:text-purple-400 italic ml-1">
                          * A despesa será lançada automaticamente todo dia{' '}
                          {formData.recurringDay || '?'} a cada{' '}
                          {formData.recurringIntervalMonths === '1'
                            ? 'mês'
                            : `${formData.recurringIntervalMonths} meses`}{' '}
                          como PENDENTE.
                        </p>
                      </div>
                    )}
                ...
                <div className="p-4 bg-primary-50/50 dark:bg-primary-900/10 rounded-2xl border border-primary-100 dark:border-primary-800/50 space-y-4">
                  <h4 className="text-[10px] font-bold text-primary-600 dark:text-primary-400 uppercase tracking-widest flex items-center gap-1.5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="col-span-2" ref={employeeDropdownRef}>
                      ...
                      <div className="relative">
                        <input
                          type="text"
                          className="w-full px-4 py-3 pr-10 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-500 outline-none transition-all font-medium text-slate-800 dark:text-slate-200 placeholder:text-slate-400"
                        ...
                        {savedEmployees.length > 0 && (
                          <button
                            ...
                            className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 text-slate-400 hover:text-primary-500 transition-colors rounded-lg hover:bg-primary-50 dark:hover:bg-primary-900/20"
                          >
                            <ChevronDown
                              className={`h-4 w-4 transition-transform duration-200 ${showEmployeeDropdown ? 'rotate-180' : ''}`}
                            />
                          </button>
                        )}
                              ...
                              !savedEmployees.includes(
                                ...
                                <div className="border-t border-slate-100 dark:border-slate-700">
                                  ...
                                  >
                                    ...
                                    Adicionar "{formData.employeeName}" como
                                    novo
                                  </button>
                                </div>
                              )}
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          type="number"
                          step="0.1"
                          className="w-full px-4 py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-500 outline-none transition-all font-mono font-bold text-slate-800 dark:text-slate-200"
                  ...
                  <div>
                    ...
                    <input
                      type="date"
                      className="w-full px-4 py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-primary-500 outline-none transition-all font-medium text-slate-800 dark:text-slate-200"
                      value={formData.commissionPaymentDate}
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          commissionPaymentDate: e.target.value,
                        })
                      }
                    />
                  </div>

                  <div className="pt-2 border-t border-primary-100/50 dark:border-primary-800/50">
                    <label className="block text-xs font-bold text-rose-500 uppercase mb-1.5 ml-1">
                      Valor Pendente a Receber (R$)
                    </label>
                    <div className="relative">
                      <span className="absolute left-4 top-1/2 -translate-y-1/2 text-rose-400 font-bold">
                        R$
                      </span>
                      <input
                        type="number"
                    ...
                    <p className="text-[10px] text-slate-400 mt-2 ml-1 italic">
                      * Se houver valor pendente, o status mudará para "PAGTO
              ...
              <div className="pt-2">
                <button
                  ...
                  type="submit"
                  className="w-full py-4 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-xl shadow-lg shadow-primary-200 dark:shadow-primary-900/30 disabled:opacity-70 transition-all transform active:scale-[0.98] flex justify-center items-center gap-2"
                >
                  {isSubmitting ? (
                    <div className="flex items-center gap-2">
