...
interface DashboardProps {
  ...
  onNavigateToTransactions: (type?: TransactionType) => void;
...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const isInMonthRange = useCallback(
    (txDate: Date) => {
  ...
  }, [transactions, selectedDate, isInMonthRange, viewMode]);
  ...
  }, [transactions, selectedDate, startDate, viewMode, monthsInRange]);
  ...
  const calculateSummary = (txs: Transaction[]) => {
    ...
    const pendingCommissions = txs
      .filter(
        (t) =>
          ...
          t.commissionPaymentDate > todayStr,
      )
      .reduce((acc, curr) => {
        if (
          curr.status === TransactionStatus.PARTIAL &&
    ...
    const futureCommissionsFromPending = txs
      ...
      .reduce((acc, curr) => {
        const proportion =
          curr.amount > 0 ? (curr.pendingAmount || 0) / curr.amount : 0;
        return acc + (curr.commissionAmount || 0) * proportion;
      }, 0);
  ...
  const previousSummary = useMemo(
    () => calculateSummary(previousTransactions),
  ...
  const topTechnicians = useMemo(() => {
    ...
    filteredTransactions.forEach((t) => {
      if (t.employeeName && t.commissionAmount) {
        ...
        if (val > 0) {
          techCommissions[t.employeeName] =
            (techCommissions[t.employeeName] || 0) + val;
        }
      }
    });
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      const dailyTx = chartContextTransactions.filter(
        (t) => t.date === dateStr,
      );
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white dark:bg-[#1E293B] backdrop-blur-xl p-4 border border-[#EEF2F7] dark:border-slate-700/60 shadow-premium-lg rounded-[14px] min-w-[180px]">
          <p className="text-[#64748B] dark:text-slate-400 text-[11px] font-medium mb-2.5 uppercase tracking-[0.08em]">
            Dia {label}
          </p>
          {payload.map((entry: any, index: number) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                ></div>
                <span className="text-[13px] font-medium text-[#64748B] dark:text-slate-300 capitalize">
                  {entry.name}
                </span>
              </div>
  ...
  return (
    <div className="space-y-8 animate-fade-in w-full min-w-0 pb-8 text-slate-800 dark:text-slate-100">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-xs font-semibold text-slate-400 mt-1">
            Acompanhe seus serviços, métricas financeiras e status da
            desentupidora.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        <div className="lg:col-span-6 2xl:col-span-4 space-y-6 flex flex-col justify-between">
          {/* Total Balance Card */}
          <div className="bg-white dark:bg-slate-800 border border-[#EEF2F7] dark:border-white/[0.06] rounded-[24px] p-6 shadow-premium relative flex-1 flex flex-col justify-between">
            <div>
              ...
              <div className="flex items-center gap-1 mt-1 text-[11px] font-bold text-emerald-500">
                ...
                <span
                  className={
                    trends.profitIsUp ? 'text-emerald-500' : 'text-rose-500'
                  }
                >
                  {trends.profit} em relação ao mês anterior
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 my-5">
              <button
                onClick={() => onNavigateToTransactions(TransactionType.INCOME)}
                className="flex items-center justify-center gap-2 bg-finexyBlack hover:bg-black text-white py-3 rounded-full text-xs font-bold transition-all shadow-sm"
              >
                <svg
              ...
              <button
                onClick={() =>
                  onNavigateToTransactions(TransactionType.EXPENSE)
                }
                className="flex items-center justify-center gap-2 bg-[#F3F4F6] hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-800 dark:text-white py-3 rounded-full text-xs font-bold transition-all"
              >
                <svg
                  className="w-3.5 h-3.5"
                  fill="none"
            ...
            <div className="pt-4 border-t border-[#EEF2F7] dark:border-white/[0.04]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                {topTechnicians.map((tech, idx) => (
                  <div
                    key={idx}
                    className="p-2 bg-slate-50 dark:bg-slate-700/50 rounded-xl border border-slate-100 dark:border-slate-800"
                  >
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] font-bold truncate max-w-[70%]">
                        {tech.name.split(' ')[0]}
                      </span>
                      <div
                        className={`w-1 h-1 rounded-full ${idx === 0 ? 'bg-finexyOrange' : 'bg-slate-300'}`}
                      ></div>
                    </div>
                    <p className="text-[11px] font-bold mt-1.5">
                      {formatCurrency(tech.total)}
                    </p>
                    <span
                      className={`text-[9px] font-bold mt-1 block ${idx === 0 ? 'text-finexyOrange' : 'text-slate-400'}`}
                    >
          ...
          <div className="bg-white dark:bg-slate-800 border border-[#EEF2F7] dark:border-white/[0.06] rounded-[24px] p-5 shadow-premium">
            <div className="flex justify-between items-center mb-2.5">
              <span className="text-xs font-bold text-slate-400">
                Saúde Financeira (Despesas x Receitas)
              </span>
            </div>
            <div className="w-full bg-slate-100 dark:bg-slate-700 h-2 rounded-full overflow-hidden mb-2">
              <div
                className={`h-full rounded-full ${currentSummary.income > 0 && currentSummary.expense / currentSummary.income > 0.7 ? 'bg-rose-500' : 'bg-finexyOrange'}`}
                style={{
                  width: `${Math.min(currentSummary.income > 0 ? (currentSummary.expense / currentSummary.income) * 100 : 0, 100)}%`,
                }}
              ></div>
            </div>
          ...
          <div className="bg-white dark:bg-slate-800 border border-[#EEF2F7] dark:border-white/[0.06] rounded-[24px] p-5 shadow-premium">
            ...
            <div className="flex gap-4 overflow-x-auto pb-1 custom-scrollbar">
                  ...
                  <div className="w-6 h-4 opacity-70">
                    ...
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M12 6v12m-3-2.818l.879.879.518.518 3.518-3.518M12 6.25l7.5 7.5-7.5-7.5"
                      ></path>
                    </svg>
                  </div>
                </div>
                <div className="mt-4">
                  <p className="text-[10px] font-mono tracking-widest text-slate-300">
                    Recebido
                  </p>
                  <p className="text-[15px] font-mono font-bold tracking-widest mt-0.5 text-white">
                    {formatCurrency(currentSummary.income)}
                  </p>
                </div>
              </div>
              ...
              <div className="min-w-[140px] bg-finexyOrange text-white p-4 rounded-[18px] flex flex-col justify-between h-[115px] relative overflow-hidden shadow-md">
                ...
                <div className="mt-4">
                  <p className="text-[10px] font-mono tracking-widest opacity-80 text-white">
                    Inadimplência
                  </p>
                  <p className="text-[15px] font-mono font-bold tracking-widest mt-0.5 text-white">
                    {formatCurrency(currentSummary.totalPendingFromClients)}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="lg:col-span-6 2xl:col-span-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="bg-finexyOrange text-white rounded-[24px] p-6 flex flex-col justify-between h-full shadow-premium-hover min-h-[170px] relative overflow-hidden group">
            ...
            <div>
              <span className="text-[clamp(1.15rem,1.75vw,1.8rem)] break-words font-semibold tracking-tight block mt-6">
                {formatCurrency(currentSummary.income)}
              </span>
          ...
          <div className="bg-white dark:bg-slate-800 border border-[#EEF2F7] dark:border-white/[0.06] rounded-[24px] p-6 flex flex-col justify-between h-full shadow-premium min-h-[170px] relative overflow-hidden group">
            ...
            <div>
              <span className="text-[clamp(1.15rem,1.75vw,1.8rem)] break-words font-semibold tracking-tight block mt-6 text-slate-800 dark:text-white">
                {formatCurrency(currentSummary.expense)}
              </span>
              <span
                className={`text-[10px] font-bold px-2 py-0.5 rounded-full inline-flex items-center gap-0.5 mt-2 ${trends.expenseIsGood ? 'text-emerald-500 bg-emerald-50 dark:bg-emerald-950/20' : 'text-rose-500 bg-rose-50 dark:bg-rose-950/20'}`}
              >
                {trends.expenseIsGood ? (
                  <ArrowDownRight className="h-3 w-3" strokeWidth={3} />
                ) : (
                  <ArrowUpRight className="h-3 w-3" strokeWidth={3} />
                )}{' '}
          ...
          <div className="bg-white dark:bg-slate-800 border border-[#EEF2F7] dark:border-white/[0.06] rounded-[24px] p-6 flex flex-col justify-between h-full shadow-premium min-h-[170px] relative overflow-hidden group">
            ...
            <div>
              ...
              <span
                className={`text-[10px] font-bold px-2 py-0.5 rounded-full inline-flex items-center gap-0.5 mt-2 ${trends.profitIsUp ? 'text-emerald-500 bg-emerald-50 dark:bg-emerald-950/20' : 'text-rose-500 bg-rose-50 dark:bg-rose-950/20'}`}
              >
                {trends.profitIsUp ? (
                  <ArrowUpRight className="h-3 w-3" strokeWidth={3} />
                ) : (
          ...
          <div className="bg-white dark:bg-slate-800 border border-[#EEF2F7] dark:border-white/[0.06] rounded-[24px] p-6 flex flex-col justify-between h-full shadow-premium min-h-[170px] relative overflow-hidden group">
              ...
              <div className="p-2 bg-slate-50 dark:bg-slate-700 rounded-xl">
                ...
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.905 0-5.64-.53-8.157-1.499"
                  ></path>
                </svg>
              </div>
            </div>
            <div>
              <span className="text-[clamp(1.15rem,1.75vw,1.8rem)] break-words font-semibold tracking-tight block mt-6 text-slate-800 dark:text-white">
                {formatCurrency(currentSummary.commissions)}
              </span>
              <div className="flex gap-2 mt-2">
                ...
                <span className="text-[10px] font-bold text-slate-500 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full inline-flex items-center gap-0.5">
                  Próximas:{' '}
                  {formatCurrency(currentSummary.futureCommissionsFromPending)}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="lg:col-span-12 2xl:col-span-4 bg-white dark:bg-slate-800 border border-[#EEF2F7] dark:border-white/[0.06] rounded-[24px] p-6 shadow-premium flex flex-col">
          <div>
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-[10px] font-semibold text-slate-400/80 mt-0.5">
                  Visão do seu faturamento no período
                </p>
              </div>
            </div>
        ...
        <div className="px-6 py-5 border-b border-[#EEF2F7] dark:border-white/[0.06] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white dark:bg-transparent">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 bg-slate-50 dark:bg-slate-700/60 border border-slate-200/50 dark:border-slate-700/30 rounded-full text-xs font-bold text-slate-600 dark:text-slate-300"
            >
              <svg
                className="w-3.5 h-3.5"
                fill="none"
              ...
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M12 3c2.755 0 5.455.477 8 1.4V14a8 8 0 01-16 0V4.4C6.545 3.477 9.245 3 12 3z"
                ></path>
              </svg>
              Filtrar
            </button>
              ...
              <thead className="text-[10px] text-slate-400 uppercase bg-slate-50/50 dark:bg-slate-800/80 border-b border-[#EEF2F7] dark:border-white/[0.04]">
                <tr>
                  <th className="px-6 py-3.5 w-12 text-center">
                    <input
                      type="checkbox"
                      className="rounded border-slate-300 text-finexyOrange focus:ring-finexyOrange"
                    />
                  </th>
              ...
              <tbody className="divide-y divide-[#EEF2F7] dark:divide-white/[0.04]">
                {filteredTransactions.slice(0, 6).map((t) => {
                  ...
                  return (
                    <tr
                      key={t.id}
                      className="hover:bg-slate-50/50 dark:hover:bg-white/[0.01] transition-colors duration-150"
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-3 py-1 rounded-full text-[10px] font-bold ${statusStyle} inline-flex items-center gap-1.5`}
                        >
                      ...
                      <td className="px-6 py-4 text-slate-400 font-semibold">
                        {day}{' '}
                        {format(parseDateLocal(t.date), 'MMM, yyyy', {
                          locale: ptBR,
                        })}
                      </td>
