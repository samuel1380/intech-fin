...
const Dashboard: React.FC<DashboardProps> = ({ transactions, onNavigateToTransactions }) => {
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      return (
        <div className="bg-white/95 backdrop-blur-md p-4 border border-slate-100 shadow-xl rounded-xl min-w-[180px]">
          <p className="text-slate-500 text-xs font-semibold mb-2 uppercase tracking-wider">Dia {label}</p>
  ...
  return (
    <div className="space-y-8 animate-fade-in w-full pb-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-center">
            ...
            <div className="bg-slate-100 p-1 rounded-xl flex border border-slate-200">
                ...
                <button 
                    ...
                    className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${viewMode === 'day' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                >
                    Diário
                </button>
            </div>
            ...
            <div className="relative min-w-[200px] group">
                ...
                <div className="flex items-center justify-between gap-3 bg-white border border-slate-200 rounded-xl px-4 py-2.5 shadow-sm group-hover:bg-slate-50 transition-colors cursor-pointer">
                    <div className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        {/* Main Flow Chart */}
        <div className="xl:col-span-2 bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-100 flex flex-col min-h-[350px]">
          <div className="flex justify-between items-start mb-8">
            ...
            <div className="flex gap-2">
                <span className="px-3 py-1 rounded-full bg-indigo-50 text-indigo-700 text-xs font-bold border border-indigo-100 uppercase tracking-wide">
                    {format(selectedDate, 'MMMM yyyy', { locale: ptBR })}
                </span>
            </div>
          </div>
        ...
        {/* Breakdown Bar Chart */}
        <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-100 flex flex-col min-h-[350px]">
          <div>
              <h3 className="text-lg font-bold text-slate-900">Entradas vs Saídas</h3>
              <p className="text-sm text-slate-500 mb-6">Comparativo de volume {viewMode === 'month' ? 'diário' : 'no mês'}.</p>
          </div>
          <div className="flex-1 w-full relative">
            {hasData ? (
                <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden w-full">
        <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-white">
            ...
            <button 
                ...
                className="text-sm text-indigo-600 hover:text-indigo-800 font-bold flex items-center group transition-colors px-4 py-2 hover:bg-indigo-50 rounded-lg"
            >
                            ...
                            <tr key={t.id} className="hover:bg-slate-50/50 transition-colors group">
                                <td className="px-8 py-5">
                                    <div className="flex flex-col">
                                        <span className="font-semibold text-slate-900 text-sm group-hover:text-indigo-600 transition-colors">{t.description}</span>
                                ...
                                <td className="px-8 py-5 text-slate-500 font-medium tabular-nums">{day}/{month}/{year}</td>
                                <td className="px-8 py-5">
...
const KPICard = ({ title, value, icon: Icon, trend, trendUp, color, subtitle, invertColor = false }: any) => {
    ...
    let trendColor = trendUp ? 'text-emerald-600 bg-emerald-50' : 'text-rose-600 bg-rose-50';
    let IconTrend = trendUp ? ArrowUpRight : ArrowDownRight;

    if (invertColor) {
        trendColor = trendUp ? 'text-rose-600 bg-rose-50' : 'text-emerald-600 bg-emerald-50';
    }
    ...
    return (
        <div className="bg-white p-6 rounded-2xl shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 border border-slate-100 flex flex-col justify-between h-full group relative overflow-hidden">
            {/* Background blob removed as per request */}
            
            <div className="flex justify-between items-start mb-2 relative z-10">
                <div className={`p-3 rounded-xl ${theme.bg} ${theme.text} transition-colors ring-1 ring-inset ring-black/5`}>
            ...
            <div className="mt-4 pt-4 border-t border-slate-50 relative z-10">
                <p className="text-xs text-slate-400 font-medium flex items-center gap-1">
                    <div className={`w-1.5 h-1.5 rounded-full ${trendUp && !invertColor ? 'bg-emerald-500' : 'bg-slate-300'}`}></div>
                    {subtitle}
                </p>
            </div>
        </div>
