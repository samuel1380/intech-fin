...
const AIAssistant: React.FC<Props> = ({ summary, transactions }) => {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-140px)]">
            ...
            {/* Chat Interface */}
            <div className="lg:col-span-2 bg-white dark:bg-slate-900/40 dark:backdrop-blur-xl rounded-2xl shadow-lg border border-slate-200 dark:border-white/5 flex flex-col overflow-hidden">
                <div className="p-4 border-b border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-slate-900/50 flex justify-between items-center">
                    <h3 className="font-semibold text-slate-700 dark:text-white flex items-center gap-2">
                        <Bot className="h-5 w-5 text-indigo-600" />
                        Consultor FinNexus
                    </h3>
                    <span className="text-xs px-2 py-1 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded-full font-medium">OpenRouter IA</span>
                </div>
                ...
                <form onSubmit={handleSendMessage} className="p-4 bg-white dark:bg-transparent border-t border-slate-100 dark:border-white/5 flex gap-2">
                    <input
                        type="text"
                        className="flex-1 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all dark:text-white dark:placeholder-slate-500"
