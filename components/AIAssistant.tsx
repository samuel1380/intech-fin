...
import { FinancialSummary, Transaction, TransactionStatus } from '../types';
...
type QuickAnalysisType =
  'cashflow' | 'employees' | 'reduction' | 'forecast' | 'clients';
...
const quickActions: QuickAction[] = [
  {
    ...
    icon: TrendingUp,
    description: 'Análise completa do fluxo',
    color: 'indigo',
  },
  {
    id: 'employees',
  ...
  {
    id: 'reduction',
    label: 'Corte de Custos',
    icon: Scissors,
    description: 'Onde economizar',
    color: 'rose',
  },
  {
    id: 'forecast',
    ...
    icon: Target,
    description: 'Previsão financeira',
    color: 'emerald',
  },
  {
    id: 'clients',
...
const AIAssistant: React.FC<Props> = ({ summary, transactions }) => {
  ...
  const [activeQuickAnalysis, setActiveQuickAnalysis] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    void getFinancialInsights(summary, transactions)
      .then((result) => {
        if (!disposed) setInsights(result);
      })
      .catch(() => {
        if (!disposed)
          setInsights('Não foi possível carregar a análise. Tente novamente.');
      })
      .finally(() => {
        if (!disposed) setLoadingInsights(false);
      });
    return () => {
      disposed = true;
    };
  ...
  const handleRefreshInsights = async () => {
    setLoadingInsights(true);
    try {
      setInsights(await getFinancialInsights(summary, transactions));
    } catch {
      notify('Não foi possível atualizar a análise.', 'error');
    } finally {
  ...
  const handleSendMessage = async (
    ...
    try {
      const response = await chatWithFinanceAI(msg, context, messages);
      setMessages((prev) => [
        ...prev,
        { role: 'ai', text: response, timestamp: new Date() },
      ]);
    } catch {
      notify('Não foi possível enviar a mensagem. Tente novamente.', 'error');
      setInputValue(msg);
    } finally {
  ...
  const handleQuickAnalysis = async (type: QuickAnalysisType) => {
    ...
    try {
      setQuickAnalysisResult(
        await generateQuickAnalysis(type, summary, transactions),
      );
    } catch {
      setQuickAnalysisResult('Análise indisponível. Tente novamente.');
    } finally {
  ...
  const colorMap: Record<string, string> = {
    indigo: 'from-primary-500 to-primary-600 shadow-primary-500/20',
    violet: 'from-primary-500 to-primary-600 shadow-primary-500/20',
  ...
  const colorBorderMap: Record<string, string> = {
    indigo:
      'border-primary-200 dark:border-primary-800/40 hover:border-primary-400 dark:hover:border-primary-600',
    violet:
      'border-primary-200 dark:border-primary-800/40 hover:border-primary-400 dark:hover:border-primary-600',
    rose: 'border-rose-200 dark:border-rose-800/40 hover:border-rose-400 dark:hover:border-rose-600',
    emerald:
      'border-emerald-200 dark:border-emerald-800/40 hover:border-emerald-400 dark:hover:border-emerald-600',
    amber:
      'border-amber-200 dark:border-amber-800/40 hover:border-amber-400 dark:hover:border-amber-600',
  };
    ...
    <div className="space-y-6 animate-fade-in pb-8">
      ...
      <div className="flex flex-col md:flex-row justify-between md:items-end gap-4 border-b border-[#EEF2F7] dark:border-slate-700/40 pb-6">
        ...
        <div className="flex gap-2">
          ...
          >
            <RefreshCw
              className={`h-4 w-4 ${loadingInsights ? 'animate-spin' : ''}`}
            />
            Atualizar Insights
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-800 rounded-[32px] p-6 border border-[#EEF2F7] dark:border-white/[0.06] shadow-premium">
          ...
          <p
            className={`text-xl font-extrabold ${summary.netProfit >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}
          >
        ...
        <div className="bg-white dark:bg-slate-800 rounded-[32px] p-6 border border-[#EEF2F7] dark:border-white/[0.06] shadow-premium">
          ...
          <p
            className={`text-xl font-extrabold ${margin >= 20 ? 'text-emerald-600 dark:text-emerald-400' : margin >= 0 ? 'text-amber-600 dark:text-amber-400' : 'text-rose-600 dark:text-rose-400'}`}
          >
            {margin.toFixed(1)}%
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-[32px] p-6 border border-[#EEF2F7] dark:border-white/[0.06] shadow-premium">
          ...
          <p
            className={`text-xl font-extrabold ${pendingCount > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-slate-800 dark:text-white'}`}
          >
            {pendingCount}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
          {quickActions.map((action) => (
            <button
              ...
              className={`group relative text-left p-4 rounded-xl border transition-all duration-200 hover:shadow-md dark:hover:shadow-none hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-wait bg-white dark:bg-[#0F172A]/80 ${colorBorderMap[action.color]} ${activeQuickAnalysis === action.id ? 'ring-2 ring-primary-500/50' : ''}`}
            >
              <div
                className={`inline-flex p-2 rounded-lg bg-gradient-to-br ${colorMap[action.color]} shadow-lg mb-3`}
              >
        ...
        <div className="bg-white dark:bg-slate-800 rounded-[32px] border border-[#EEF2F7] dark:border-white/[0.06] shadow-premium overflow-hidden animate-fade-in">
          <div className="px-6 py-4 border-b border-[#EEF2F7] dark:border-white/[0.06] flex items-center justify-between bg-slate-50 dark:bg-slate-900/50">
            <div className="flex items-center gap-3">
              <div
                className={`p-2 rounded-lg bg-gradient-to-br ${colorMap[quickActions.find((a) => a.id === activeQuickAnalysis)?.color || 'indigo']} shadow-lg`}
              >
                {(() => {
                  const action = quickActions.find(
                    (a) => a.id === activeQuickAnalysis,
                  );
              ...
              <div>
                <h3 className="font-semibold text-[#0F172A] dark:text-white">
                  {
                    quickActions.find((a) => a.id === activeQuickAnalysis)
                      ?.label
                  }
                </h3>
            ...
            {loadingQuickAnalysis ? (
              <div className="flex flex-col items-center justify-center py-16 gap-4">
                <div className="relative">
                  ...
                  <div className="w-12 h-12 border-4 border-primary-600 rounded-full border-t-transparent animate-spin absolute inset-0"></div>
                </div>
                <p className="text-[13px] text-[#64748B] dark:text-slate-400 animate-pulse font-medium">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 min-h-[500px]">
        ...
        <div className="lg:col-span-1 bg-gradient-to-br from-primary-900 via-primary-950 to-slate-900 rounded-2xl shadow-xl text-white p-6 overflow-y-auto flex flex-col">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-primary-300/60 font-medium mt-0.5">
                  Powered by {getActiveProviderName()}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-6 space-y-3">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-300 mt-1">
                      Você tem {formatCurrency(summary.pendingInvoices)} em
                      faturas pendentes. Considere implementar um sistema de
                      lembrete automático.
                    </p>
                  </div>
                </div>
              ...
              <div className="p-4 bg-rose-500/10 rounded-xl border border-rose-500/20">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-300 mt-1">
                      Sua margem de lucro está em {margin.toFixed(1)}%. O ideal
                      para o setor é acima de 20%. Revise seus custos
                      operacionais.
                    </p>
                  </div>
                </div>
            ...
            {(summary.pendingCommissions || 0) > 0 && (
              <div className="p-4 bg-primary-500/10 rounded-xl border border-primary-500/20">
                <div className="flex items-start gap-3">
                  <Users className="h-5 w-5 text-primary-400 shrink-0 mt-0.5" />
                  <div>
                    <h4 className="font-semibold text-primary-200 text-sm">
                      Comissões a Pagar
                    </h4>
                    <p className="text-xs text-slate-300 mt-1">
                      {formatCurrency(summary.pendingCommissions || 0)} em
                      comissões agendadas. Reserve esse valor no caixa.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-slate-800 rounded-[32px] shadow-premium border border-[#EEF2F7] dark:border-white/[0.06] flex flex-col overflow-hidden">
          <div className="p-5 border-b border-[#EEF2F7] dark:border-white/[0.06] bg-slate-50 dark:bg-slate-900/50 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              <span
                className="text-xs px-2 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 rounded-full font-medium"
                title={`Modelo: ${getActiveModelName()}`}
              >
                ⚡ {getActiveProviderName()}
              </span>
              ...
              >
                ...
                >
                  ...
                  <div>
                    ...
                    {m.timestamp && (
                      <p
                        className={`text-[10px] text-slate-400 mt-1 ${m.role === 'user' ? 'text-right' : 'text-left'}`}
                      >
                        {m.timestamp.toLocaleTimeString('pt-BR', {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
                      </p>
            ...
            {chatLoading && (
              <div className="flex justify-start">
                <div className="flex gap-3">
                  <div className="shrink-0 w-8 h-8 rounded-full flex items-center justify-center bg-gradient-to-br from-primary-500 to-primary-600 text-white shadow-lg shadow-primary-500/20 text-xs">
                    🤖
                  </div>
              ...
              <div className="flex flex-wrap gap-2">
                {suggestedQuestions.slice(0, 4).map((q, idx) => (
                  <button
                    ...
                    className="text-xs px-3 py-1.5 bg-slate-100 dark:bg-slate-800/60 text-slate-600 dark:text-slate-300 rounded-full hover:bg-primary-50 dark:hover:bg-primary-900/20 hover:text-primary-600 dark:hover:text-primary-400 transition-colors border border-[#EEF2F7] dark:border-slate-700/40 font-medium"
                  >
                    {q}
                  </button>
                ))}
          ...
          <form
            ...
            className="p-4 bg-white dark:bg-[#0d1526]/50 border-t border-slate-100 dark:border-slate-700/40 flex gap-2"
          >
            <input
              type="text"
              className="flex-1 bg-slate-50 dark:bg-white/[0.04] border border-[#EEF2F7] dark:border-slate-700 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-500 transition-all dark:text-white dark:placeholder-slate-500 text-sm"
            ...
            <button
              ...
              className="bg-primary-600 hover:bg-primary-700 text-white p-3 rounded-xl transition-all disabled:opacity-50 shadow-lg shadow-primary-500/20 hover:shadow-primary-500/30 active:scale-95"
            >
