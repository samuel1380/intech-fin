...
const Reports: React.FC<Props> = ({ transactions, taxSettings }) => {
  ...
  const totalCommissions = filteredTransactions
    ...
    .reduce((acc, curr) => {
      if (
        ...
        const proportion =
          curr.amount > 0 ? Math.max(0, receivedAmount / curr.amount) : 0;
  ...
  const totalExpenseForPie = Object.values(expenseDataMap).reduce(
    (s, v) => s + v,
    0,
  );
    ...
    .map((key) => ({
      ...
      percentage:
        totalExpenseForPie > 0
          ? ((expenseDataMap[key] / totalExpenseForPie) * 100).toFixed(1)
          : '0',
    }))
  ...
  const barData = [
    ...
    { name: 'Comissões', value: totalCommissions, fill: '#f59e0b' },
    {
      name: 'Líquido',
      value: netAfterTax,
      fill: netAfterTax >= 0 ? '#6366f1' : '#ef4444',
    },
  ];
  ...
  const handleDownloadPDF = () => {
    ...
    doc.setFontSize(10);
    doc.text(`Gerado em: ${format(new Date(), 'dd/MM/yyyy HH:mm')}`, 195, 20, {
      align: 'right',
    });
    ...
    doc.text(
      `Impostos Estimados (${(totalTaxRate * 100).toFixed(1)}%)`,
      19,
      secondRowY + 8,
    );
    doc.setFontSize(14);
    ...
    const tableRows = filteredTransactions.map((t) => {
      ...
      return [
        ...
        t.employeeName || '-',
        t.commissionAmount
          ? `${formatBRL(displayComm)}${isPartial ? ' (Prop.)' : ''}`
          : '-',
        formattedCommDate,
      ];
    });
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text(
        `Página ${i} de ${pageCount} - FinNexus Enterprise System`,
        105,
        290,
        { align: 'center' },
      );
    ...
    doc.save(
      `Relatorio_Financeiro_FinNexus_${format(new Date(), 'yyyy-MM-dd')}.pdf`,
    );
  };
  ...
  return (
    <div className="space-y-8 w-full animate-fade-in pb-8">
      ...
      <div className="flex flex-col lg:flex-row justify-between lg:items-center gap-6 border-b border-slate-200 dark:border-slate-700/40 pb-6">
        <div>
          <h2 className="text-3xl font-semibold text-[#0F172A] dark:text-white tracking-tight">
            Relatórios & Auditoria
          </h2>
          <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm font-medium">
            Emissão de documentos e análise fiscal.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
          <div className="bg-slate-100 dark:bg-slate-800 p-1.5 rounded-full inline-flex w-max border border-slate-200 dark:border-slate-700">
            <button
              ...
              className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${viewMode === 'month' ? 'bg-white dark:bg-slate-700 text-primary-600 dark:text-primary-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
            >
              Mensal
            </button>
            <button
              onClick={() => setViewMode('day')}
              className={`px-5 py-2 rounded-full text-sm font-bold transition-all ${viewMode === 'day' ? 'bg-white dark:bg-slate-700 text-primary-600 dark:text-primary-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'}`}
            >
              Diário
            </button>
          </div>
          ...
          {viewMode === 'month' ? (
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div className="relative group min-w-[140px]">
                <div className="flex items-center justify-between gap-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 shadow-sm group-hover:bg-slate-50 dark:group-hover:bg-slate-700/50 transition-colors cursor-pointer">
                  <div className="flex items-center gap-1.5">
                    <CalendarRange className="h-4 w-4 text-primary-500 shrink-0" />
                    <span className="text-slate-700 dark:text-slate-200 font-bold text-sm capitalize">
                      {format(parseISO(`${endMonth}-01`), 'MMM yyyy', {
                        locale: ptBR,
                      })}
                    </span>
                  </div>
            ...
            <input
              ...
              className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 shadow-sm font-bold text-sm outline-none focus:ring-2 focus:ring-primary-500 text-slate-700 dark:text-slate-200"
            />
          )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-[32px] p-5 border border-[#EEF2F7] dark:border-white/[0.06] shadow-premium">
          ...
          <p className="text-xl font-bold text-amber-600 dark:text-amber-400">
            {formatCompact(totalCommissions)}
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-[32px] p-5 border border-[#EEF2F7] dark:border-white/[0.06] shadow-premium">
          ...
          <p className="text-xl font-bold text-yellow-600 dark:text-yellow-400">
            {formatCompact(estimatedTax)}
          </p>
          <p className="text-[10px] text-slate-400 font-medium mt-1">
            {(totalTaxRate * 100).toFixed(1)}% sobre receita
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-[32px] p-5 border border-[#EEF2F7] dark:border-white/[0.06] shadow-premium">
          ...
          <p
            className={`text-xl font-bold ${netAfterTax >= 0 ? 'text-primary-600 dark:text-primary-400' : 'text-rose-600 dark:text-rose-400'}`}
          >
        ...
        <div className="bg-white dark:bg-slate-800 rounded-[32px] p-5 border border-[#EEF2F7] dark:border-white/[0.06] shadow-premium">
          <div className="flex items-center gap-2 mb-2">
            <div className="p-1.5 bg-primary-50 dark:bg-primary-900/30 rounded-lg">
              <Percent className="h-4 w-4 text-primary-600 dark:text-primary-400" />
            </div>
            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">
              Margem
            </span>
          ...
          <p
            className={`text-lg font-extrabold ${profitMargin >= 20 ? 'text-emerald-600 dark:text-emerald-400' : profitMargin >= 0 ? 'text-amber-600 dark:text-amber-400' : 'text-rose-600 dark:text-rose-400'}`}
          >
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    Clique em um funcionário para ver o relatório completo
                  </p>
                </div>
              </div>
          ...
          <div className="divide-y divide-[#EEF2F7] dark:divide-white/[0.04]/30">
            {employees.map((emp, idx) => (
              <button
                ...
                onClick={() =>
                  setSelectedEmployee(
                    selectedEmployee === emp.name ? null : emp.name,
                  )
                }
                className={`w-full p-4 md:p-5 flex items-center gap-4 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-all text-left group
                                    ${selectedEmployee === emp.name ? 'bg-primary-50/50 dark:bg-primary-900/10' : ''}`}
              >
                {/* Avatar */}
                <div
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    {emp.jobCount} {emp.jobCount === 1 ? 'serviço' : 'serviços'}{' '}
                    • Média {formatBRL(emp.avgPerJob)}/serviço
                  </p>
                </div>
                ...
                <div className="hidden md:flex items-center gap-6">
                  <div className="text-right">
                    ...
                    <p className="text-sm font-extrabold text-emerald-600 dark:text-emerald-400">
                      {formatCompact(emp.totalRevenue)}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] font-bold text-slate-400 uppercase">
                    ...
                    <p className="text-sm font-extrabold text-amber-600 dark:text-amber-400">
                      {formatCompact(emp.totalCommission)}
                    </p>
                  </div>
                  {emp.pendingCommission > 0 && (
                    <div className="text-right">
                      ...
                      <p className="text-sm font-extrabold text-rose-500">
                        {formatCompact(emp.pendingCommission)}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <ChevronRight
                  className={`h-5 w-5 text-slate-300 dark:text-slate-600 transition-transform ${selectedEmployee === emp.name ? 'rotate-90' : 'group-hover:translate-x-0.5'}`}
                />
              </button>
            ))}
          </div>
      ...
      {selectedEmp && (
        <div
          className="fixed inset-0 bg-black/50 dark:bg-black/70 backdrop-blur-sm z-50 flex items-start justify-center p-4 pt-[5vh] overflow-y-auto"
          onClick={() => setSelectedEmployee(null)}
        >
          <div
            className="bg-white dark:bg-[#0d1526] rounded-2xl shadow-2xl dark:shadow-none border border-slate-200 dark:border-slate-700/40 w-full max-w-3xl max-h-[85vh] overflow-y-auto animate-fade-in"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Header */}
            <div className="sticky top-0 bg-gradient-to-r from-primary-600 to-primary-700 p-6 rounded-t-2xl text-white z-10">
              <div className="flex items-center justify-between">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="bg-emerald-50 dark:bg-emerald-900/20 p-4 rounded-xl border border-emerald-100 dark:border-emerald-800/30">
                  ...
                  <p className="text-xl font-extrabold text-emerald-700 dark:text-emerald-300">
                    {formatBRL(selectedEmp.totalRevenue)}
                  </p>
                </div>
                <div className="bg-amber-50 dark:bg-amber-900/20 p-4 rounded-xl border border-amber-100 dark:border-amber-800/30">
                  ...
                  <p className="text-xl font-extrabold text-amber-700 dark:text-amber-300">
                    {formatBRL(selectedEmp.totalCommission)}
                  </p>
                  {selectedEmp.commissionRate > 0 && (
                    <p className="text-[10px] text-amber-500 font-medium mt-0.5">
                      Taxa: {selectedEmp.commissionRate}%
                    </p>
                  )}
                </div>
                <div className="bg-primary-50 dark:bg-primary-900/20 p-4 rounded-xl border border-primary-100 dark:border-primary-800/30">
                  ...
                  <p className="text-xl font-extrabold text-primary-700 dark:text-primary-300">
                    {selectedEmp.jobCount}
                  </p>
                  <p className="text-[10px] text-primary-400 font-medium mt-0.5">
                    Média: {formatBRL(selectedEmp.avgPerJob)}
                  </p>
                </div>
                <div
                  className={`p-4 rounded-xl border ${
                    selectedEmp.totalVales > 0
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <TrendingDown className="h-4 w-4 text-rose-600 dark:text-rose-400" />
                    <span
                      className={`text-[10px] font-bold uppercase ${selectedEmp.totalVales > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-slate-400'}`}
                    >
                      Vales / Despesas
                    </span>
                  </div>
                  <p
                    className={`text-xl font-extrabold ${selectedEmp.totalVales > 0 ? 'text-rose-700 dark:text-rose-300' : 'text-slate-300 dark:text-slate-600'}`}
                  >
              ...
              <div>
                <h4 className="text-sm font-bold text-slate-800 dark:text-white mb-3 flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-slate-400 ml-auto">
                    {selectedEmp.transactions.length} registros
                  </span>
                </h4>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-slate-50 dark:bg-slate-800/50 text-left">
                        <th className="px-4 py-3 text-[11px] font-bold text-slate-400 uppercase">
                          Data
                        </th>
                        <th className="px-4 py-3 text-[11px] font-bold text-slate-400 uppercase">
                          Descrição
                        ...
                        <th className="px-4 py-3 text-[11px] font-bold text-slate-400 uppercase text-right">
                          Valor
                        </th>
                        <th className="px-4 py-3 text-[11px] font-bold text-slate-400 uppercase text-right">
                          Comissão
                      ...
                      {selectedEmp.transactions
                        ...
                        .map((t, i) => (
                          ...
                          >
                            ...
                            <td className="px-4 py-3 text-slate-800 dark:text-white font-medium max-w-[200px] truncate">
                              {t.description}
                            </td>
                            ...
                            <td className="px-4 py-3 text-xs text-slate-500 dark:text-slate-400 whitespace-nowrap">
                              {t.commissionPaymentDate
                                ? formatDate(t.commissionPaymentDate)
                                : '-'}
                            </td>
                          </tr>
                        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {hasData && pieData.length > 0 ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-4">
                ...
                <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">
                  Adicione despesas para gerar a análise gráfica.
                </p>
              </div>
            )}
          </div>
        ...
        <div className="space-y-6">
          {/* Auditoria Fiscal */}
          <div className="bg-gradient-to-br from-primary-900 via-primary-950 to-slate-900 p-8 rounded-2xl shadow-xl text-white relative overflow-hidden">
            <div className="relative z-10">
              <h3 className="text-xl font-bold mb-1">Auditoria Automática</h3>
              <p className="text-primary-200/70 text-sm mb-6">
                Análise fiscal ref. ao período selecionado (
                {filteredTransactions.length} registros).
              </p>
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white/10 backdrop-blur-sm p-4 rounded-xl border border-white/10">
                  <span className="text-[11px] text-primary-300/80 uppercase tracking-wider font-bold">
                    Lucro Bruto
                  </span>
                  <div
                    className={`text-xl font-extrabold mt-1 ${grossProfit >= 0 ? 'text-white' : 'text-rose-400'}`}
                  >
                    {formatBRL(grossProfit)}
                  </div>
                </div>
                <div className="bg-white/10 backdrop-blur-sm p-4 rounded-xl border border-white/10">
                  <span className="text-[11px] text-amber-300/80 uppercase tracking-wider font-bold">
                    Impostos ({(totalTaxRate * 100).toFixed(1)}%)
                  </span>
                ...
                <div className="bg-white/10 backdrop-blur-sm p-4 rounded-xl border border-white/10">
                  <span className="text-[11px] text-emerald-300/80 uppercase tracking-wider font-bold">
                    Resultado Líquido
                  </span>
                  <div
                    className={`text-xl font-extrabold mt-1 ${netAfterTax >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
                  >
                ...
                <div className="bg-white/10 backdrop-blur-sm p-4 rounded-xl border border-white/10">
                  ...
                  <div
                    className={`text-xl font-extrabold mt-1 ${pendingTotal > 0 ? 'text-amber-400' : 'text-emerald-400'}`}
                  >
            ...
            <div className="absolute top-0 right-0 w-64 h-64 bg-primary-600 rounded-full blur-3xl opacity-20 -mr-16 -mt-16"></div>
                ...
                >
                  ...
                  >
                    ...
                    <LabelList
                      dataKey="value"
                      position="right"
                      formatter={(val) => formatCompact(Number(val) || 0)}
                      style={{
                        fontSize: 11,
                        fontWeight: 700,
                        fill: isDark ? '#94a3b8' : '#64748b',
