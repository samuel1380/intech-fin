...
const Accounts: React.FC<Props> = ({
  ...
  const getDaysUntil = useCallback(
    (dateStr: string) => {
      ...
      const diffTime = date.getTime() - todayTime;
  ...
  }, [receivables, payables, getDaysUntil]);

  // Totais
  const totalReceivable = receivables.reduce(
    (s, t) => s + (t.pendingAmount || t.amount),
    0,
  );
  ...
  const renderTransactionRow = (t: Transaction) => (
    <div
      key={t.id}
      className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 bg-white dark:bg-slate-800 rounded-[24px] border border-transparent hover:border-slate-100 dark:hover:border-slate-700/50 hover:shadow-sm transition-all group"
    >
        ...
        <button
          ...
          className={`shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold transition-all border shadow-sm ${
            t.type === TransactionType.INCOME
              ...
              : 'bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-400 border-primary-200 dark:border-primary-800/50 hover:bg-primary-100 dark:hover:bg-primary-900/40'
          } disabled:opacity-50`}
          title={
            t.type === TransactionType.INCOME
    ...
    <div className="space-y-6 animate-fade-in w-full pb-8">
      ...
      <div className="border-b border-[#EEF2F7] dark:border-slate-700/40 pb-6">
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-1 font-medium">
          Contas a pagar, receber e alertas de vencimento.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {/* Alertas urgentes */}
        <div
          className={`p-6 rounded-[32px] shadow-premium transition-all cursor-pointer ${
            activeTab === 'alerts'
        ...
        >
          <div className="flex items-center gap-3 mb-3">
            <div className="p-2 rounded-lg bg-amber-100 dark:bg-amber-900/30">
              <Bell
                className={`h-5 w-5 text-amber-600 dark:text-amber-400 ${alerts.length > 0 ? 'animate-bounce' : ''}`}
              />
            </div>
          ...
          <div className="text-2xl font-semibold text-[#0F172A] dark:text-white">
            {alerts.length}
          </div>
          <div className="flex gap-2 mt-1.5 flex-wrap">
            ...
            {alerts.length - overdueCount - todayCount > 0 && (
              <span className="text-[10px] font-bold text-blue-600 dark:text-blue-400">
                {alerts.length - overdueCount - todayCount} próxima
                {alerts.length - overdueCount - todayCount > 1 ? 's' : ''}
              </span>
            )}
            {alerts.length === 0 && (
        ...
        >
          ...
          <p className="text-[10px] font-medium text-slate-400 mt-1">
            ...
            pendente{receivables.length !== 1 ? 's' : ''}
          </p>
        </div>
        ...
        >
          ...
          <p className="text-[10px] font-medium text-slate-400 mt-1">
            {payables.length} conta{payables.length !== 1 ? 's' : ''} pendente
            {payables.length !== 1 ? 's' : ''}
          </p>
        </div>
        ...
        <div className="p-6 rounded-[32px] bg-white dark:bg-slate-800 border border-[#EEF2F7] dark:border-white/[0.06] shadow-premium">
          ...
          <div
            className={`text-2xl font-bold ${totalReceivable - totalPayable >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}
          >
          ...
          <>
              ...
              <div className="p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/40 rounded-xl flex items-center gap-3 animate-fade-in">
                ...
                <div>
                  ...
                  <p className="text-xs text-red-600 dark:text-red-400">
                    Você tem {overdueCount} conta{overdueCount > 1 ? 's' : ''}{' '}
                    vencida{overdueCount > 1 ? 's' : ''}. Regularize o mais
                    rápido possível.
                  </p>
                </div>
              </div>
              ...
              <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/40 rounded-xl flex items-center gap-3 animate-fade-in">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-600 dark:text-amber-400">
                    {todayCount} conta{todayCount > 1 ? 's' : ''} vence
                    {todayCount > 1 ? 'm' : ''} hoje.
                  </p>
                </div>
              </div>
            ...
            {filterBySearch(alerts).length > 0 ? (
              ...
              <div className="p-12 text-center bg-white dark:bg-slate-800 rounded-[32px] border border-[#EEF2F7] dark:border-white/[0.06] shadow-sm">
                ...
                <h3 className="font-semibold text-[#0F172A] dark:text-slate-200 mb-1">
                  Tudo em dia!
                </h3>
                <p className="text-sm text-slate-400">
                  Nenhuma conta com vencimento próximo (7 dias). 🎉
                </p>
              </div>
            )}
          </>
